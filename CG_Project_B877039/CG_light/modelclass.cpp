...
ModelClass::ModelClass(ID3D11Device* device, const WCHAR* modelFilename, const WCHAR* textureFilename1, const WCHAR* textureFilename2, vector<XMFLOAT3> instancePosition, int RandScail, XMFLOAT3 modelScail)
{
	m_RandScail = RandScail;

	m_modelScail = modelScail;
	m_instancePosition = instancePosition;
	this->Initialize(device, modelFilename, textureFilename1, textureFilename2);
...
{
	return m_instancePosition.size();
}
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	instances = new InstanceType[m_instancePosition.size()]; 
	if (!instances) { return false; }
	for (int i = 0; i < m_instancePosition.size(); i++)
	{
		instances[i].position = m_instancePosition.at(i);
	}

	instanceBufferDesc.Usage = D3D11_USAGE_DEFAULT; 
	instanceBufferDesc.ByteWidth = sizeof(InstanceType) * m_instancePosition.size();
