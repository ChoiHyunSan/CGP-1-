...
void LightShaderClass::LightSet(int input)
{
	if (m_lightSet[input] == 0.0f) m_lightSet[input] = 1.0f;
	else m_lightSet[input] = 0.0f;
}
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, 
	ID3D11ShaderResourceView** textureArray, 
	XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower, XMFLOAT4 diffuseColors[], XMFLOAT4 lightPositions[])
{
	HRESULT result;
	...
	dataPtr2->lightType = XMFLOAT4(m_lightSet[0] , m_lightSet[1], m_lightSet[2], 1.0f);
